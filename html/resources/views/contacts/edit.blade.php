...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Editar Contacto</h1>
        <p class="text-gray-600">Atualize o contacto de {{ $contact->person->name }}</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('contacts.update', $contact->id) }}" method="POST">
            ...
            <div class="flex items-center justify-between">
                <a href="{{ route('people.show', $contact->person_id) }}" class="text-gray-600 hover:text-gray-800 font-medium">
                    <i class="fas fa-arrow-left mr-2"></i>Voltar para Pessoa
                </a>
                <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-lg font-medium">
                    <i class="fas fa-save mr-2"></i>Atualizar Contacto
                </button>
            </div>
        </form>
    </div>
